...
$(() => {
  ...
  const currentPage = Math.round(skip / limit) + 1;

  let pagination = '';
  pagination += "<li class='page-item " + (currentPage === 1 ? 'disabled' : '') + "'>";
  pagination += "<a class='page-link' href='#' data-page='" + (currentPage - 1) + "'>Previous</a>";
  pagination += '</li>';
  ...
  pagination += "<a class='page-link' href='#' data-page='" + (currentPage + 1) + "'>Next</a>";
  pagination += '</li>';
...
$(() => {
  ...
  $tableWrapper.scroll(function () {
    ...
    document.querySelector('#fadeToWhiteID').style.opacity = Math.min(Math.max(proximityToRightOfTable - 50, 50) - 50, 100) / 100;
  });
  ...
  $('.pagination .page-link').on('click', function (event) {
    event.preventDefault();
    const { limit, skip, totalPages } = ME_SETTINGS;
    let currentPage = Math.round(skip / limit) + 1;
    const page = Number.parseInt($(this).data('page'), 10);
    if (!Number.isNaN(Number(page)) && page >= 1 && page <= totalPages && page !== currentPage) {
      currentPage = page;
      const searchParams = new URLSearchParams(window.location.search);
      searchParams.set('skip', (currentPage * limit) - limit);
